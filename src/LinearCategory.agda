...
module notation (R : CommRing 𝓥) (str : category-structure 𝓤 𝓥) where
  ...
  addition M = module-on._+_ M

  syntax addition is-mod x y = x +⟨ is-mod ⟩ y

  smul : {H : 𝓥 ̇ } → module-on R H → ⟨ R ⟩ → H → H
  smul M = module-on._·_ M

  syntax smul is-mod x y = x ·⟨ is-mod ⟩ y
...
record linear-precategory-axioms (R : CommRing 𝓥) (str : category-structure 𝓤 𝓥) : 𝓤 ⊔ 𝓥 ̇ where
  ...
  field
    ...
    linearL : ∀ {A B C : ob} → (a b : hom A B) → (c : hom B C) →
      (a +⟨ homMod A B ⟩ b) ⨾ c ＝ (a ⨾ c) +⟨ homMod A C ⟩ (b ⨾ c)
    linearR : ∀ {A B C : ob} → (a : hom A B) → (b c : hom B C) →
      a ⨾ (b +⟨ homMod B C ⟩ c) ＝ (a ⨾ b) +⟨ homMod A C ⟩ (a ⨾ c)
    compatible : ∀ {A B C : ob} → (r s : ⟨ R ⟩) → (a : hom A B) (b : hom B C) →
      (r ·⟨ homMod A B ⟩ a) ⨾ (s ·⟨ homMod B C ⟩ b) ＝ (r *ᴿ s) ·⟨ homMod A C ⟩ (a ⨾ b)
